...
describe("additional funs", async () => {
  ...
  it("TEST mint", async () => {
    ...
    await expect(contract.connect(user).mint(user.address, 1)).to.be.revertedWith("not an owner");
  });

  it("TEST burn", async () => {
    expect(`${await contract.totalSupply()}`).to.equal("10000000");

    await contract.connect(owner).burn(owner.address, 1_000_000);
    expect(`${await contract.totalSupply()}`).to.equal("9000000");

    await expect(contract.connect(user).burn(user.address, 1)).to.be.revertedWith("not an owner");
  });
});
